...
const Component9: React.FC<Props> = (props)  => {
    ...
    const setDateCommissioned = (_event,date: Date) => {
  ...
  const handleCreateItem = async (itemToCreate: MuseumItemType) => {
    ...
    try {
      const response = await fetch('http://10.2.59.118:1337/museum-items',
      {
        method: 'POST',
        headers: {
        ...
        <View>
            ...
            <View>
                ...
                <TouchableWithoutFeedback onPress = {()=>setState({...state, showDatePicker: true})}>
                    <Text>
                        {state.museumItem.dateCommissioned?state.museumItem.dateCommissioned.toLocaleDateString("en-gb",{weekday: 'short'}):'--Press here to select date--'} 
                    </Text>
                </TouchableWithoutFeedback>
                {
                    state.showDatePicker && 
                    <DateTimePicker value={state.museumItem.dateCommissioned !== null? state.museumItem.dateCommissioned: new Date()}
                    mode={'date'}
                    is24Hour={true}
                    display="default"
